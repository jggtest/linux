...
static_assert(same_memory(struct rlist_cpu_pages_state, position,
				...
				struct rlist_cpu_entry *entry)
{
	...
	struct page **first = rlscpu->rls_pages.next_entry;
	struct page **cur;
	size_t npages = 1;

	rlscpu->rls_pages.position = (rlscpu->rls_pages.next_entry -
	...
	rlscpu->rls_pages.next_entry = cur;
	return true;
}
...
bool rlscpu_reset(struct rlist_cpu_state *rlscpu, struct rlist_cpu_entry *entry)
{
	...
	case RLIST_CPU_PAGES: {
		rlscpu->rls_pages.next_entry = rlscpu->rlist_cpu->pages.pages;
...
int rlscpu_append_begin(struct rlist_cpu_state_append *rlsa)
{
	...
	case RLIST_CPU_PAGES:
		rlsa->rls_pages.next_entry = rlsa->rlist_cpu->pages.pages;
		return true;
	default:
		WARN(true, "Corrupt rlist_cpu");
		return false;
...
int rlscpu_append_folio_pages(struct rlist_cpu_pages_state *rlspages,
			      ...
			      size_t length)
{
	...
	if (rlspages->rlist_cpu->pages.pages +
		    rlspages->rlist_cpu->pages.available - rlspages->next_entry <
	    length / PAGE_SIZE)
		return -ENOSPC;

	page = folio_page(folio, offset / PAGE_SIZE);
	for (;length; length -= PAGE_SIZE) {
		*rlspages->next_entry = page;
		page = nth_page(page, 1);
	}
	return 0;
}
